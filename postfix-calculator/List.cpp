...
void List::makeEmpty(){
  ListItr nodeCurrent = first();
  while(!nodeCurrent.isPastEnd()){
    int currentValue = nodeCurrent.current->value;
    remove(currentValue);//Use remove() defined below to remove the current node
    nodeCurrent.moveForward(); //Reser the current node to next
  }
}
